...
/*
 * = 모든 클래스를 등록하는것이 아니다 (기능(컴포넌트)을 가지고 있는 클래스 => 여러 클래스에서 호출)
 *   1) 등록에서 제외 : 데이터형(자바 기본데이터형, 사용자 정의 데이터형: ~VO)
 *   2) 생성 / 소멸부분은 스프링 자체 담당
 *   3) 특별한 경우가 아니면 싱글톤(한개의 메모리주소를 이용해서 사용)
 *                     DI/AOP
 *                     MVC
 *                     Transaction
 *                     보안(spring-security)
...
public class MainClass {
	 ...
	 *    
	 *    메소드 호출이 가능
	 *      = 객체 생성 => init-method="메소드명"
	 *      = 객체 소멸 => destroy-method="메소드명"
	 *      
	 *    XML에 등록
	 *    1) 사용자 정의 클래스
	 *       = 패키지단위로 등록 => Annotation
	 *    2) 라이브러리 클래스 등록 => MyBatis, OXM, MVC(ViewResolver)
	 *       = <bean>를 이용해서 필요한 클래스만 등록
	 */
